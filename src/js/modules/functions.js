...
export function focusTrap(e, el, key, focus = true, target = false) {
	...
	let index = focusElement.indexOf(e.target);
	let direction = key ? -1 : 1;
	let length = focusElement.length;
...
export function scrollLockToggle() {
	...
	lockPaddingElements.forEach(el => { el.style.paddingRight = paddingOffset; });
